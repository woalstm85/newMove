...
class RegularMoveTypeModal extends StatefulWidget {
  const RegularMoveTypeModal({super.key});

  @override
  _RegularMoveTypeModalState createState() => _RegularMoveTypeModalState();
}

class _RegularMoveTypeModalState extends State<RegularMoveTypeModal> {
  ...
  Future<void> _loadSelectedMoveType() async {
    ...
    setState(() {
      selectedMoveType = prefs.getString('selectedRegularMoveType');
    });
  }
  ...
  Future<void> _saveSelectedMoveType(String type) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('selectedRegularMoveType', type);
  }
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                // 원래 코드에서는 Padding 안에 Row가 있어서 한 번 더 패딩이 들어감
                // 이제 모달의 좌우 패딩(24px)과 동일하게 버튼들이 배치됨
